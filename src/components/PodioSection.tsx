...
const PodioSection = () => {
  return <section className="relative py-24 overflow-hidden bg-space-dark/90 hidden">
      <div className="container mx-auto px-4">
        ...
        once: true
      }} className="text-center text-5xl md:text-6xl font-black text-space-light mb-20 tracking-wider hidden">
          PODIO Orbital
        </motion.h2>
        ...
        <div className="relative flex justify-center">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            {podioItems.map((item, i) => <motion.div key={item.letter} custom={i} initial="hidden" whileInView="visible" viewport={{
            once: true,
            amount: 0.1
          }} variants={orbitVariants} className={`absolute w-24 h-24 md:w-28 md:h-28 rounded-full bg-gradient-to-br ${item.color} 
                          shadow-lg flex flex-col items-center justify-center
                          cursor-pointer transition-transform hover:scale-110`} style={{
        ...
        {/* Descriptions */}
        <div className="mt-32 max-w-3xl mx-auto space-y-8 hidden">
          {podioItems.map((item, i) => <motion.div key={`desc-${item.letter}`} initial={{
          opacity: 0,
          x: i % 2 === 0 ? -50 : 50
        }} whileInView={{
          opacity: 1,
